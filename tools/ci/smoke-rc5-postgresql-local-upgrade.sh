...
bundle="${NETRATEL_RC5_LOCAL_COMPOSE_BUNDLE:?set NETRATEL_RC5_LOCAL_COMPOSE_BUNDLE}"
legacy_api="${NETRATEL_RC5_LEGACY_API_IMAGE:?set NETRATEL_RC5_LEGACY_API_IMAGE}"
legacy_migrations="${NETRATEL_RC5_LEGACY_MIGRATIONS_IMAGE:?set NETRATEL_RC5_LEGACY_MIGRATIONS_IMAGE}"
legacy_web="${NETRATEL_RC5_LEGACY_WEB_IMAGE:?set NETRATEL_RC5_LEGACY_WEB_IMAGE}"
current_api="${NETRATEL_RC5_CURRENT_API_IMAGE:?set NETRATEL_RC5_CURRENT_API_IMAGE}"
current_migrations="${NETRATEL_RC5_CURRENT_MIGRATIONS_IMAGE:?set NETRATEL_RC5_CURRENT_MIGRATIONS_IMAGE}"
current_web="${NETRATEL_RC5_CURRENT_WEB_IMAGE:?set NETRATEL_RC5_CURRENT_WEB_IMAGE}"
project="netratel-rc5-local-upgrade-${GITHUB_RUN_ID:-local}-${RANDOM}"
web_port="${NETRATEL_RC5_LOCAL_WEB_PORT:-18082}"
web_url="http://127.0.0.1:${web_port}"
api_port="${NETRATEL_RC5_LOCAL_API_PORT:-18083}"
api_url="http://127.0.0.1:${api_port}"
bundle_dir="$(mktemp -d)"
key_dir="$(mktemp -d)"
cookie_jar="$(mktemp)"
key_path="$key_dir/agent-auth-private.pem"
stage="preparing rc.5 PostgreSQL/local upgrade"
...
compose=(docker compose --project-name "$project" -f "$bundle_dir/compose.images.yaml" \
  -f "$root/tests/compose/rc5-local-upgrade-api.compose.yaml")

cleanup() {
  local status=$?
  if (( status != 0 )); then
    echo "::error title=rc.5 PostgreSQL/local upgrade smoke failed::${stage}" >&2
...
export POSTGRES_PASSWORD=synthetic-rc5-local-upgrade-postgres-password
...
cli_executable="${NETRATEL_RC5_LOCAL_CLI_EXECUTABLE:-$root/src/NetRatel/NetRatel.Cli/bin/Release/net10.0/netratel}"
if [[ -n "${NETRATEL_RC5_LOCAL_CLI_ARCHIVE:-}" ]]; then
  [[ -s "$NETRATEL_RC5_LOCAL_CLI_ARCHIVE" ]] || { echo "Review CLI archive is missing." >&2; exit 1; }
  cli_dir="$(mktemp -d)"
  tar -xzf "$NETRATEL_RC5_LOCAL_CLI_ARCHIVE" -C "$cli_dir"
  cli_executable="$cli_dir/netratel-cli-linux-x64/netratel"
fi
[[ -x "$cli_executable" ]] || { echo "Review CLI executable is missing." >&2; exit 1; }

stage="starting immutable published rc.5 PostgreSQL/local images"
configure_images "$legacy_api" "$legacy_migrations" "$legacy_web"
"${compose[@]}" up --detach
wait_for_migrations
wait_for_state '.setupRequired == true and .isReady == false'

stage="initializing an rc.5 local administrator from a private synthetic file"
...
echo "Published rc.5 PostgreSQL/local upgrade continuity passed."
