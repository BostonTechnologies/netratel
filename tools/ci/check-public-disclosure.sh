...
while IFS= read -r -d '' path; do
  case "$path" in
    .dockerignore|.env.example|.gitattributes|.gitignore|CODE_OF_CONDUCT.md|CONTRIBUTING.md|Directory.Build.props|Directory.Build.targets|LICENSE|NetRatel.sln|NOTICE|README.md|SECURITY.md|build-main.ps1|compose.yaml|coverage.runsettings|dotnet-install.sh|dotnet-tools.json|global.json|readme.trustmodel.md|setup-dotnet9.sh|setup.sh|.github/*|docker/*|docs/*|release/*|samples/*|scripts/*|src/NetRatel/*|tests/*|tools/*)
      ;;
    *)
