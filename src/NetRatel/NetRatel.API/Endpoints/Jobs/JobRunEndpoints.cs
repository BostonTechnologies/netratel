...
public static class JobRunEndpoints
{
    ...
    {
        var group = app.MapGroup("/api/v1/jobruns").WithTags("Job Runs").RequireAuthorization("Operator");
        group.MapPost("/start/{jobId:long}", async (ulong jobId, [FromBody] RunJobRequest request, IAkkaJobAuthorityService authority, IEventRecorder events, ICorrelationContext correlation, CancellationToken ct) =>
        {
            try
            {
        ...
        group.MapPost("/{id:long}/cancel", async (ulong id, IAkkaJobAuthorityService authority, IJobRunService runs, CancellationToken ct) =>
        {
        ...
        group.MapDelete("/{id:long}", async (ulong id, IJobRunService runs, CancellationToken ct) => { if (await runs.GetAsync(id, ct) is null) return Results.NotFound(); await runs.DeleteAsync(id, ct); return Results.NoContent(); });
        group.MapGet("/", async (IJobRunService runs, IJobDefinitionService jobs, ITenantService tenants, [FromServices] OrchestratorDbContext db, [FromQuery] JobRunStatusDto? status, [FromQuery] ulong? jobId, [FromQuery] int? tenantId, [FromQuery] int? take, CancellationToken ct) =>
        {
            var list = await BuildAsync(runs, jobs, tenants, db, ct);
        ...
        group.MapGet("/{id:long}/steps", async (ulong id, IJobRunService runs, CancellationToken ct) =>
        {
        ...
        group.MapGet("/{id:long}/steps/{ordinal:int}/logs", async (ulong id, int ordinal, IJobRunService runs, CancellationToken ct) =>
        {
