...
{
    Task<IReadOnlyList<AccessTenantDto>> GetTenantsAsync(CancellationToken cancellationToken = default);
    Task<IReadOnlyList<AccessRoleDto>> GetRolesAsync(int? tenantId, CancellationToken cancellationToken = default);
    Task<EffectiveAccessSummaryDto> GetSelfAsync(CancellationToken cancellationToken = default);
    Task<IReadOnlyList<LocalUserAccessDto>> GetUsersAsync(int? tenantId, CancellationToken cancellationToken = default);
    ...
    Task<IReadOnlyList<RoleAssignmentDto>> GetAssignmentsAsync(string principalId, int? tenantId, CancellationToken cancellationToken = default);
...
public sealed class AccessAdministrationApiService(IHttpClientFactory clients) : IAccessAdministrationApiService
{
    ...
    public async Task<IReadOnlyList<AccessTenantDto>> GetTenantsAsync(CancellationToken cancellationToken = default) =>
        await Client.GetFromJsonAsync<List<AccessTenantDto>>("/api/v2/access/tenants", cancellationToken).ConfigureAwait(false) ?? [];

    public async Task<IReadOnlyList<AccessRoleDto>> GetRolesAsync(int? tenantId, CancellationToken cancellationToken = default) =>
        await Client.GetFromJsonAsync<List<AccessRoleDto>>($"/api/v2/access/roles{Query(tenantId)}", cancellationToken).ConfigureAwait(false) ?? [];
    ...
    public async Task<IReadOnlyList<LocalUserAccessDto>> GetUsersAsync(int? tenantId, CancellationToken cancellationToken = default) =>
        await Client.GetFromJsonAsync<List<LocalUserAccessDto>>($"/api/v2/access/users{Query(tenantId)}", cancellationToken).ConfigureAwait(false) ?? [];
    ...
    public async Task<IReadOnlyList<RoleAssignmentDto>> GetAssignmentsAsync(string principalId, int? tenantId, CancellationToken cancellationToken = default) =>
        await Client.GetFromJsonAsync<List<RoleAssignmentDto>>($"/api/v2/access/principals/{Uri.EscapeDataString(principalId)}/assignments{Query(tenantId)}", cancellationToken).ConfigureAwait(false) ?? [];
    ...
    private static string Query(int? tenantId) => tenantId is int id ? $"?tenantId={id}" : string.Empty;
}

public sealed record AccessTenantDto(int TenantId, string Name);
